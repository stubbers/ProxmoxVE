#!/usr/bin/env bash
source <(curl -s https://raw.githubusercontent.com/stubbers/ProxmoxVE/refs/heads/librespeed/misc/build.func)
...
function update_script() {
  ...
  if [[ ! -f /opt/${APP}_version.txt ]] || [[ "${RELEASE}" != "$(cat /opt/${APP}_version.txt)" ]]; then
    ...
    temp_file=$(mktemp)
    curl -fsSL "https://github.com/librespeed/speedtest-rust/releases/download/${RELEASE}/librespeed-rs-x86_64-unknown-linux-gnu.deb" -o "$temp_file"
    dpkg -u "$temp_file"
    rm -rf "$temp_file"
